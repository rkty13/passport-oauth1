 ...
 *   - `requestTokenMethod`    HTTP Method used to obtain unauthorized request token. Default is POST.
 *   - `accessTokenURL`        URL used to exchange a user-authorized request token for an access token
 *   - `accessTokenMethod`     HTTP Method used to exchange a user-authorized request token for an access token. Default is POST.
...
function OAuthStrategy(options, verify) {
  ...
  this._oauth.setClientOptions({
    'requestTokenHttpMethod': options.requestTokenMethod || 'POST',
    'accessTokenHttpMethod': options.accessTokenMethod || 'POST'   
  });
}
...
OAuthStrategy.prototype.authenticate = function(req, options) {
  ...
  if (req.query && req.query.oauth_token) {
    ...
    if (callbackURL) {
      ...
      // console.log("parsed callbackURL: ", parsed);
      // if (!parsed.protocol) {
    ...
    this._oauth.getOAuthRequestToken(params, function(err, token, tokenSecret, params) {
      ...
      function stored(err) {
        if (err) { return self.error(err); }
        console.log("self._userAuthorizationURL: ", self._userAuthorizationURL);
        var parsed = url.parse(self._userAuthorizationURL, true);
        console.log("parsed self._userAuthorizationURL: ", parsed);
        ...
        // console.log("formatted parsed: ", location);
        var location = "https://www.schoology.com/oauth/authorize?oauth_token=" + token + "&oauth_callback=https://staging.usebask.com/auth/schoology/callback";
...
OAuthStrategy.prototype.userProfile = function(token, tokenSecret, params, done) {
  console.log("func.userProfile: ", token, tokenSecret, params, done);
  return done(null, {});
};
...
OAuthStrategy.prototype._loadUserProfile = function(token, tokenSecret, params, done) {
  var self = this;
  console.log("func._loadUserProfile: ", token, tokenSecret, params);
